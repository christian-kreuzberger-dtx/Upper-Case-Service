...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == Upper-Case-Service
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "Upper-Case-Service" {
	...
	// Get SLI File from Upper-Case-Service subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=Upper-Case-Service/sli.yaml
	sliFile := "Upper-Case-Service/sli.yaml"
